  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/50 rounded-xl p-4 space-y-1 shadow-sm">
          ...
          <span className="text-[11px] text-slate-400 dark:text-slate-500 font-mono">총 {storage.length}개 스토리지 풀 활성</span>
        </div>
        ...
        <div className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/50 rounded-xl p-4 space-y-1 shadow-sm">
          ...
          <span className="text-[11px] text-slate-400 dark:text-slate-500 font-mono">전체 사용률: {totalPercent}%</span>
        </div>
        ...
        <div className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/50 rounded-xl p-4 space-y-1 shadow-sm">
          ...
          <span className="text-[11px] text-slate-400 dark:text-slate-500 font-mono">안전 여유치 확보됨</span>
        </div>
        ...
        <div className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/50 rounded-xl p-4 space-y-1 shadow-sm">
          ...
          <div className="text-xl font-bold font-mono text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4 text-emerald-500 dark:text-emerald-400" />
            <span>정상 (HEALTHY)</span>
          </div>
          <span className="text-[11px] text-slate-400 dark:text-slate-500 font-mono">LVM-Thin / ZFS / Dir</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {storage.map((pool) => {
          ...
          return (
            <div key={`${pool.node}-${pool.storage}`} className="rounded-xl p-5 space-y-4 border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/50 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                  <div
                    className="w-9 h-9 rounded-lg flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700/60"
                  >
                    <HardDrive className="w-4 h-4" />
                  </div>
                  <div>
                    <h4 className="font-semibold text-sm text-slate-900 dark:text-slate-100">{pool.storage}</h4>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">{pool.node || '모든 노드'}</span>
                  </div>
                </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 dark:text-slate-400">사용량 ({percent}%)</span>
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 space-y-1.5">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider block">허용 콘텐츠</span>
