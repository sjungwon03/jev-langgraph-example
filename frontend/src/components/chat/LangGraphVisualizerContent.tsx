...
export function LangGraphVisualizerContent({
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-950">
      {/* Header Toolbar */}
      {showHeader && (
        <div className="p-4 border-b border-slate-800/90 flex flex-row items-center justify-between bg-slate-950/80 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center border border-blue-500/30 shadow-inner">
              <GitFork className="w-4 h-4" />
            </div>
            <div>
              <div className="text-sm font-semibold text-slate-100 flex items-center gap-2">
                <span>LangGraph StateGraph 워크플로우 아키텍처</span>
                <Badge variant="outline" className="text-[10px] py-0 px-1.5 font-mono text-blue-300 border-blue-800/80 bg-blue-950/40">
                  State Machine
                </Badge>
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={loadGraph}
              disabled={loading}
              className="h-8 border-slate-800 bg-slate-900 text-slate-300 hover:text-slate-100 px-2.5"
            >
      ...
      <div className="flex-1 flex overflow-hidden">
        {viewTab === 'VISUAL' ? (
          <>
            ...
            <div className="w-80 border-l border-slate-800 bg-slate-950 p-5 flex flex-col justify-between shrink-0 overflow-y-auto">
              <div className="space-y-4">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800/80">
                  <span className="text-xs font-bold text-slate-200">노드 상세 속성</span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-xs font-bold text-slate-100">{selectedNode.name}</h4>
                      </div>
                      <p className="text-xs text-slate-400 leading-relaxed bg-slate-900/60 p-2.5 rounded-lg border border-slate-800/70">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {selectedNode.stateChanges.map((sc) => (
                            <Badge
                              key={sc}
                              variant="outline"
                              className="font-mono text-[10px] bg-slate-900 border-slate-700/80 text-blue-300"
                            >
                              {sc}
                            </Badge>
                          ))}
                    ...
                    <div className="space-y-2 pt-2 border-t border-slate-800/80">
                      <label className="text-[11px] font-medium text-slate-300 block">연결된 엣지 및 전이 조건</label>
                      <div className="space-y-1.5">
                        {graphData?.edges
                          ...
                          .map((e, idx) => (
                            <div
                              key={idx}
                              className="p-2 rounded bg-slate-900/50 border border-slate-800/80 text-[11px] space-y-1 font-mono"
                            >
              ...
              <div className="pt-4 border-t border-slate-800 text-[11px] text-slate-500 flex items-center justify-between">
                ...
                <span className="font-mono text-emerald-400">Compiled OK</span>
              </div>
            </div>
          </>
        ) : viewTab === 'MERMAID' ? (
          ...
          <div className="flex-1 p-6 flex flex-col bg-slate-950 overflow-hidden">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button
                ...
                className="h-7 text-xs border-slate-800 bg-slate-900 text-slate-300 hover:text-white flex items-center gap-1.5"
              >
                {copied ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                <span>{copied ? '복사됨' : 'Mermaid 코드 복사'}</span>
              </Button>
            </div>
            <pre className="flex-1 p-4 rounded-xl bg-slate-900 border border-slate-800 text-xs font-mono text-slate-200 overflow-auto whitespace-pre leading-relaxed">
          ...
          <div className="flex-1 p-6 flex flex-col bg-slate-950 overflow-y-auto space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xs font-bold text-slate-200">InfraAgentState (Annotation.Root)</h3>
                <p className="text-[11px] text-slate-400">노드 간 누적/전달되는 LangGraph 세션 상태 데이터 구조</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-blue-400">messages</span>
                  <Badge variant="outline" className="text-[10px] font-mono">BaseMessage[]</Badge>
                </div>
                <p className="text-[11px] text-slate-400">대화 히스토리 및 사용자 메시지 (reducer: concat)</p>
              </div>

              <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-blue-400">intent</span>
                  <Badge variant="outline" className="text-[10px] font-mono">string</Badge>
                </div>
                <p className="text-[11px] text-slate-400">LLM이 분류한 최종 작업 의도 식별자</p>
              </div>

              <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-blue-400">toolToCall</span>
                  <Badge variant="outline" className="text-[10px] font-mono">ToolCall | null</Badge>
                </div>
                <p className="text-[11px] text-slate-400">실행할 도구명(name) 및 파라미터(args)</p>
              </div>

              <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-blue-400">confirmationNeeded</span>
                  <Badge variant="outline" className="text-[10px] font-mono">Confirmation | null</Badge>
                </div>
                <p className="text-[11px] text-slate-400">파괴적 고위험 작업 시 발급된 Human-in-the-Loop 토큰</p>
              </div>

              <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-blue-400">toolResult</span>
                  <Badge variant="outline" className="text-[10px] font-mono">any</Badge>
                </div>
                <p className="text-[11px] text-slate-400">Proxmox MCP 또는 자원 신청/승인 실행 결과 JSON</p>
              </div>

              <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-blue-400">finalResponse</span>
                  <Badge variant="outline" className="text-[10px] font-mono">string</Badge>
                </div>
                <p className="text-[11px] text-slate-400">사용자에게 스트리밍되는 최종 마크다운 대화 답변</p>
              </div>
            </div>
          </div>
        )}
